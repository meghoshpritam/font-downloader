...
def get_css_font_configs(file_content):
    configs = []
    config_texts = list(
        filter(lambda x: len(x) > 10, file_content.split('/*')))
    for config_text in config_texts:
        lines = config_text.split('\n')
        configs.append({
            "comment": lines[0].replace('*/', '').strip(),
            "fontFamily": re.sub(r"[;'\"]", '',  lines[2].replace('font-family:', '')).strip(),
            "fontStyle":  re.sub(r"[;'\"]", '',  lines[3].replace('font-style:', '')).strip(),
            "fontWeight": re.sub(r"[;'\"]", '',  lines[4].replace('font-weight:', '')).strip(),
            "fontStretch": re.sub(r"[;'\"]", '',  lines[5].replace('font-stretch:', '')).strip(),
            "fontDisplay": re.sub(r"[;'\"]", '',  lines[6].replace('font-display:', '')).strip(),
            "src": lines[7].split('(')[1].split(')')[0],
            "format": re.sub(r"[;'\"]", '', lines[7].split('format(')[1].split(')')[0]),
            "unicodeRange": re.sub(r"[;'\"]", '',  lines[8].replace('unicode-range:', '')).strip(),
        })
...
def generate_css_font_config(configs, static_prefix, use_font_weight=True, use_font_stretch=False):
    content = ''
    for config in configs:
...
def generate_css_font_compact_config(configs, static_prefix):
    ...
    return generate_css_font_config(minified_config, static_prefix, use_font_weight=False)
...
def main():
    ...
    write_file(output_file_name.replace(
        '.css', '-compact.css'), content_to_save_compact)
